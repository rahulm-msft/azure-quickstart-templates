...
resource networkFabricProvision 'Microsoft.Resources/deploymentScripts@2020-10-01' = {
  ...
  properties: {
    //scriptContent: 'az extension add --source ${wheelFileURL} -y; az nf fabric provision -g "${resourceGroup().name}" --resource-name "${networkFabricsName}"; echo ${resourceGroup().name}; echo ${networkFabricsName}; sleep 5m'
    scriptContent: loadTextContent('provision.sh')
    azCliVersion: '2.42.0'
