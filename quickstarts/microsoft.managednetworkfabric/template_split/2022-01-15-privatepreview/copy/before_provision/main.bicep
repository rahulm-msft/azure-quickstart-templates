...
//   scope: subscription()
//   name: 'b3a10629-b7ad-4469-949d-c4463121700d'
// }
...
resource updateSerialNumber 'Microsoft.Resources/deploymentScripts@2020-10-01' = [for i in range(0, deviceCount): {
  ...
  properties: {
    scriptContent: 'az extension add --source ${wheelFileURL} -y; response=$(az nf device show -g ${resourceGroup().name} --resource-name "${deviceNameList[i]}" --query "{role:networkDeviceRole, sku:networkDeviceSku}"); role=$(echo "$response" | grep role | cut -d\\" -f4); sku=$(echo "$response" | grep sku | cut -d\\" -f4);  az nf device update --resource-group ${resourceGroup().name}  --location ${location}  --resource-name "${deviceNameList[i]}" --serial-number "${serialNumberList[i]}" --network-device-sku "$sku" --network-device-role "$role"; if [ $((${deviceCount}-1)) -eq ${i} ]; then result=$(az nf fabric provision -g ${resourceGroup().name} --resource-name ${networkFabrics.name}); fi'
    azCliVersion: '2.42.0'
