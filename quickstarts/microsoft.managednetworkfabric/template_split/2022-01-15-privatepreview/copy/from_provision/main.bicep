...
resource roleAssign 'Microsoft.Authorization/roleAssignments@2020-04-01-preview' = {
  ...
  properties: {
    roleDefinitionId: roleDefinitionId //ownerRoleDefinition.id // once managed identity is created, assign the 'Owner' permission to it.
...
resource l2DomainUpdateAdminState 'Microsoft.Resources/deploymentScripts@2020-10-01' = [for i in range(0, l2DomainCount): {
  ...
  properties: {
    scriptContent: 'az extension add --source ${wheelFileURL} -y; az nf l2domain update-admin-state --resource-name "${l2DomainsName[i]}" --resource-group ${resourceGroup().name} --state "Enable"; az nf l2domain update-admin-state --resource-name "${l2DomainsName[i]}" --resource-group ${resourceGroup().name} --state "Disable"'
    azCliVersion: '2.42.0'
...
resource l3DomainUpdateAdminState 'Microsoft.Resources/deploymentScripts@2020-10-01' = [for i in range(0, l3DomainCount): {
  ...
  properties: {
    scriptContent: 'az extension add --source ${wheelFileURL} -y; az nf l3domain update-admin-state --resource-name "${l3DomainsName[i]}" --resource-group ${resourceGroup().name} --state "Enable"; az nf l3domain update-admin-state --resource-name "${l3DomainsName[i]}" --resource-group ${resourceGroup().name} --state "Disable"'
    azCliVersion: '2.42.0'
    retentionInterval: 'PT4H'
  }
}]
